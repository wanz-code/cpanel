...
async function CreatePanel(sock, paket, username, nomorRaw) {
  ...
  const email = `${username}@WanzOfficial.com`
...
/*
 case 'cpanel': {
  try {
    if (!text) return m.reply(`Format: ${prefix}cpanel paket,username,nomor\nContoh: ${prefix}cpanel unli,wanz,6281234567890`)

    let [paket, username, nomorRaw] = text.split(",")
    if (!paket || !username || !nomorRaw) return m.reply(`Format salah! Gunakan: ${prefix}cpanel paket,username,nomor`)

    paket = paket.toLowerCase()
    const specs = ['1gb','2gb','3gb','4gb','5gb','6gb','7gb','8gb','9gb','unli']
    if (!specs.includes(paket)) return m.reply(`Paket tidak valid! Pilih: ${specs.join(", ")}`)

    const result = await CreatePanel(sock, paket, username, nomorRaw)

    // Pesan sukses ke chat publik
    await m.reply(`✅ PANEL BERHASIL DIBUAT!\n• Username: ${result.username}\n• Paket: ${result.paket}\n• Server ID: ${result.serverId}`)

  } catch (err) {
    console.error(err)
    m.reply(`❌ Gagal membuat panel: ${err.message || JSON.stringify(err)}`)
  }
}
break
